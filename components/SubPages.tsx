...
import { 
  ...
  HelpCircle, CreditCard, BarChart2, Eye, Archive
} from 'lucide-react';
import { useNav } from '../App';
import { CURRENT_USER, MOCK_CHAT_HISTORY, MOCK_COMMUNITY_FEED, MOCK_COMMENTS, MOCK_PRODUCTS, MOCK_TRANSACTIONS, Message, FOLLOWED_FRIENDS, MOCK_MY_COMMENTS, MOCK_VIDEOS, MOCK_PROJECTS, MOCK_CHANNELS } from '../types';
    ...
    <div className="flex flex-col h-full bg-gray-100">
      ...
      <div className="h-14 bg-white border-b flex items-center justify-between px-4 shrink-0 shadow-sm z-10">
        ...
        <div className="flex flex-col items-center">
          ...
          {params.count && <span className="text-[10px] text-gray-500">({params.count} members)</span>}
        </div>
        <button 
        ...
        {messages.map((msg) => (
          ...
          >
            ...
            {msg.type === 'system' ? (
              ...
              >
                 {/* Avatar */}
                 <img src={msg.avatar} alt={msg.sender} className="w-10 h-10 rounded-lg object-cover shadow-sm mx-2" />
...
export const ProductManagementPage = () => {
   ...
   // Mock data with local state for reactivity
   ...
   const updateStatus = (id: string, newStatus: 'on_shelf' | 'off_shelf') => {
      setProductList(prev => prev.map(p => {
         if (p.id === id) {
             // Side-effect to update global mock data so other screens see the change
            ...
            {displayProducts.length === 0 ? (
               ...
               displayProducts.map(p => (
                  <div key={p.id} className="bg-white rounded-xl p-3 shadow-sm flex items-start space-x-3">
                     <img src={p.image} className="w-20 h-20 rounded-lg object-cover bg-gray-100" />
      ...
      <div className="flex flex-col h-full bg-gray-50">
         ...
         <div className="flex-1 overflow-y-auto p-4 space-y-4">
            ...
            <div className="bg-white p-4 rounded-xl flex flex-col items-center justify-center border-2 border-dashed border-gray-200 h-40 relative overflow-hidden">
               {existingProduct ? (
                  <img src={existingProduct.image} className="absolute inset-0 w-full h-full object-cover opacity-50" />
               ) : null}
            ...
            <div className="bg-white rounded-xl p-4">
               ...
               <div className="space-y-4">
                  ...
                  <div className="text-xs text-gray-400 leading-relaxed bg-gray-100 p-2 rounded">
                     <p>交易说明：</p>
                     <p>1. 平台针对橱窗商品采用双层收费模式。</p>
                     <p>2. 当你定价 {sellerPrice || 3000} 仪豆时，系统会自动加上管理费，售价为 {displayPrice || 3300} 仪豆。</p>
                     <p>3. 消费者支付 {displayPrice || 3300} 仪豆后，你将实际到账 {sellerPrice || 3000} 仪豆。</p>
                  </div>
               </div>
            </div>
    ...
    <div className="flex flex-col h-full bg-white relative">
       ...
       <div className="flex-1 overflow-y-auto -mt-4 bg-white rounded-t-3xl relative z-10 px-4 pt-6 pb-20">
          ...
          <div className="flex items-center space-x-3 mb-6 bg-gray-50 p-3 rounded-xl">
             <img src={product.seller.avatar} className="w-12 h-12 rounded-full" />
       ...
       <div className="absolute bottom-0 w-full bg-white border-t p-2 flex items-center space-x-2 pb-safe z-20">
          {isSeller ? (
             ...
             <div className="flex w-full space-x-3 px-2">
                ...
                <button className="flex-1 bg-gray-100 text-gray-900 py-3 rounded-full font-bold text-sm flex items-center justify-center">
                   <BarChart2 size={16} className="mr-1" /> 数据
                </button>
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4">
         ...
         <div className="bg-orange-50 border border-orange-100 p-3 rounded-lg flex items-start space-x-2">
            ...
            <div className="text-xs text-orange-700 leading-relaxed">
               <span className="font-bold">提现说明：</span> 平台默认按照 10:1 方式进行仪豆金额转换。例如：提现 1000 元人民币，需扣除 10,000 仪豆。真实到账时会扣除第三方服务费（如提现 1000 元扣除 20 元服务费）。
            </div>
         </div>
     ...
     <div className="flex flex-col h-full bg-white">
        ...
        <div className="flex-1 overflow-y-auto p-5">
           ...
           <div className="flex items-center space-x-3 mb-6">
              <img src={article.user.avatar} className="w-8 h-8 rounded-full" />
           ...
           {article.images && <img src={article.images[0]} className="w-full rounded-lg mb-6" />}
           <div className="prose prose-sm text-gray-800 leading-7">
              <p>{article.content}</p>
              <p>这里是文章详情内容的占位符。更多详细内容会在这里展示。这里是文章详情内容的占位符。更多详细内容会在这里展示。</p>
           </div>
        </div>
            ...
            <div className="bg-white p-4">
               ...
               <div className="border-b pb-4 mb-4">
                  <div className="flex items-center space-x-2 mb-2">
                     <img src="https://picsum.photos/id/1011/50/50" className="w-6 h-6 rounded-full" />
                     <span className="text-sm font-medium text-gray-800">仪表老司机</span>
                  </div>
                  <p className="text-sm text-gray-800 leading-relaxed mb-2">这种情况一般是因为电极内的KCL溶液自然挥发导致的。建议先用蒸馏水清洗电极，然后浸泡在3mol/L的KCL溶液中活化24小时。</p>
      ...
      <div className="flex flex-col h-full bg-gray-50">
         ...
         <div className="p-4">
            ...
            <div className="space-y-4">
               ...
               <div className="flex items-center p-4 bg-white rounded-lg cursor-pointer">
                  <div className="bg-green-500 p-2 rounded-full text-white mr-4"><Smartphone size={20} /></div>
                  <div className="flex-1">
                     <div className="font-medium">手机联系人</div>
                     <div className="text-xs text-gray-400">添加通讯录中的朋友</div>
                  </div>
                  <ChevronLeft className="rotate-180 text-gray-300" />
               </div>
            </div>
         ...
         <div className="flex-1 overflow-y-auto">
            ...
            {FOLLOWED_FRIENDS.map(f => (
               ...
               >
                  ...
                  <img src={f.avatar} className="w-10 h-10 rounded-lg mr-3" />
                  <span className="text-gray-900 font-medium">{f.name}</span>
               </div>
            ))}
            
            {/* Mock More Contacts */}
            <div className="bg-gray-100 px-4 py-1 text-xs text-gray-500 font-bold">A</div>
            {[1, 2, 3].map(i => (
               <div 
                  key={`a-${i}`} 
                  className="flex items-center px-4 py-3 border-b border-gray-50 cursor-pointer"
                  onClick={() => toggleSelect(`a-${i}`)}
               >
                  <div className="mr-4">
                     {selected.includes(`a-${i}`) ? 
                        <CheckCircle className="text-green-500 fill-current" /> : 
                        <Circle className="text-gray-300" />
                     }
                  </div>
                  <div className="w-10 h-10 rounded-lg mr-3 bg-gray-200" />
                  <span className="text-gray-900 font-medium">Alex {i}</span>
               </div>
            ))}
         </div>
      </div>
      ...
      <div className="h-48 w-full relative">
         ...
         <div className="absolute bottom-4 right-4 flex space-x-2">
            {isOwner ? (
               <button 
                  onClick={() => {}}
                  className="bg-white text-blue-600 px-3 py-1.5 rounded-full text-xs font-bold"
               >
                  管理
               </button>
            ) : (
      ...
      <div className="flex-1 overflow-y-auto">
         {activeTab === 'home' && (
            <div className="p-4 space-y-4">
               ...
               {/* Feed Preview */}
               <div className="bg-white p-4 rounded-xl shadow-sm">
                  <h3 className="font-bold text-gray-900 mb-3">频道动态</h3>
                  {MOCK_COMMUNITY_FEED.slice(0, 3).map(item => (
                     <div key={item.id} className="flex items-start py-3 border-b border-gray-50 last:border-0" onClick={() => pushScreen({ name: 'community_detail', params: { postId: item.id } })}>
            ...
            <div className="p-4 space-y-3">
               {[1,2,3].map(i => (
                  <div key={i} className="bg-white p-4 rounded-xl shadow-sm flex items-center">
                     ...
                     <button className="text-blue-600 text-xs border border-blue-600 px-2 py-0.5 rounded">下载</button>
                  </div>
               ))}
            </div>
         )}
...
const MenuItem = ({ label, isSwitch = false }: { label: string, isSwitch?: boolean }) => (
   <div className="flex items-center justify-between px-4 py-3.5 border-b border-gray-50 active:bg-gray-50">
      <span className="text-base text-gray-900">{label}</span>
      {isSwitch ? (
         <div className="w-10 h-6 bg-gray-200 rounded-full relative"><div className="w-5 h-5 bg-white rounded-full shadow absolute top-0.5 left-0.5"></div></div>
      ) : (
    ...
    <div className="flex flex-col h-full bg-gray-50">
      ...
      <div className="p-6 flex-1 overflow-y-auto">
        ...
        <div className="space-y-4 bg-white p-4 rounded-xl shadow-sm">
          ...
          <div className="flex flex-col space-y-1">
             <label className="text-sm font-medium text-gray-700">频道类型 <span className="text-red-500">*</span></label>
             <div className="relative">
                <select className="w-full bg-gray-50 rounded-lg p-3 text-sm appearance-none focus:outline-none">
                  <option>在线分析仪</option>
                  <option>技术交流</option>
                  <option>行业资讯</option>
                </select>
      ...
      <div className="flex-1 p-4 overflow-y-auto">
         {!searchTerm && (
           <>
             ...
             {/* Discover */}
             <div>
               <h3 className="text-sm font-bold text-gray-900 mb-3">搜索指定内容</h3>
               <div className="flex flex-wrap gap-4 text-sm text-blue-600">
                 <span className="w-1/4 text-center py-2">朋友圈</span>
                 <span className="w-1/4 text-center py-2">文章</span>
                 <span className="w-1/4 text-center py-2">公众号</span>
                 <span className="w-1/4 text-center py-2">小程序</span>
                 <span className="w-1/4 text-center py-2">音乐</span>
                 <span className="w-1/4 text-center py-2">表情</span>
               </div>
             </div>
           </>
         )}
         ...
         {searchTerm && (
           <div className="text-center mt-10 text-gray-400 text-sm">
             搜索 "{searchTerm}" ...
             <div className="mt-2 text-xs text-gray-300">Mock Search Result: No items found.</div>
           </div>
         )}
      </div>
    </div>
...
export const CommunityDetail = ({ params }: { params: { postId: string } }) => {
  ...
  return (
    <div className="flex flex-col h-full bg-white relative">
      ...
      <div className="flex-1 overflow-y-auto bg-gray-50 pb-safe">
        <div className="bg-white p-4 mb-2">
           ...
           {post.images && post.images.map((img, i) => (
             <img key={i} src={img} className="w-full h-auto rounded-lg mb-2" alt="post content" />
           ))}
                  ...
                  <div className="flex-1">
                     <div className="flex justify-between items-start">
                        ...
                        <div className="flex flex-col items-center">
                           ...
                           <span className="text-[10px] text-gray-400">{comment.likes || '赞'}</span>
                        </div>
                     </div>
      ...
      <div className="bg-white border-t p-3 pb-safe flex items-center space-x-3">
         <div className="flex-1 bg-gray-100 rounded-full px-4 py-2 text-sm text-gray-400 flex items-center">
           <span className="mr-2 text-gray-500">✎</span> 说点什么...
         </div>
      ...
      <div className="flex flex-col h-full bg-black">
         ...
         <div className="absolute top-0 w-full p-4 flex justify-between items-center z-10">
            ...
            <div className="flex items-center space-x-4">
               ...
               <button className="bg-black/20 backdrop-blur-md p-2 rounded-full text-white"><MoreHorizontal size={20} /></button>
            </div>
         </div>

         {/* Video Area */}
         <div className="flex-1 flex items-center justify-center bg-gray-900 relative">
             <img src={video.videoInfo?.cover} className="w-full h-full object-contain opacity-80" />
             ...
             <div className="absolute bottom-0 w-full p-4 bg-gradient-to-t from-black/80 via-black/40 to-transparent">
                <div className="flex items-center space-x-2 mb-2">
                   <img src={video.user.avatar} className="w-10 h-10 rounded-full border border-white/50" />
                   <div className="text-white">
                      <div className="font-bold text-sm">@{video.user.name}</div>
                      <div className="text-xs opacity-80">2小时前</div>
                   </div>
                   <button className="bg-red-500 text-white text-xs px-3 py-1 rounded-full ml-auto">关注</button>
                </div>
                <p className="text-white text-sm mb-4 leading-relaxed line-clamp-2">{video.content}</p>
                <div className="flex items-center justify-between text-white/80">
                   <div className="flex items-center space-x-6">
                      <div className="flex flex-col items-center"><Heart size={24} /><span className="text-xs mt-1">{video.stats.likes}</span></div>
                      <div className="flex flex-col items-center"><MessageSquare size={24} /><span className="text-xs mt-1">{video.stats.comments}</span></div>
                      <div className="flex flex-col items-center"><Share2 size={24} /><span className="text-xs mt-1">{video.stats.shares}</span></div>
                   </div>
                </div>
             </div>
         </div>
         ...
         <div className="h-16 bg-gray-900 border-t border-gray-800 flex items-center px-4 space-x-3">
   ...
   return (
      <div className="flex flex-col h-full bg-white">
         ...
         <div className="flex-1 overflow-y-auto">
            ...
            <div className="p-5">
               <div className="flex items-center space-x-2 mb-3">
                  <span className="bg-blue-600 text-white text-xs px-2 py-0.5 rounded-sm font-bold">
                     {project.projectInfo?.status.toUpperCase()}
                  </span>
                  {project.tags?.map(t => <span key={t} className="bg-gray-100 text-gray-600 text-xs px-2 py-0.5 rounded">{t}</span>)}
               </div>
               ...
               <div className="flex items-center justify-between text-sm text-gray-500 mb-6 pb-6 border-b">
                  <div className="flex items-center">
                     ...
                     <span>{project.projectInfo?.location || '远程'}</span>
                  </div>
                  <div className="flex items-center">
                  ...
                  <div className="flex items-center">
                     ...
                     <span>{project.projectInfo?.roles?.length || 0} 岗位</span>
                  </div>
               </div>

               {/* Funding/Progress Card */}
               {project.projectInfo?.progress !== undefined && (
                  <div className="bg-gray-50 rounded-xl p-4 mb-6">
                     <div className="flex justify-between items-end mb-2">
                        <div className="text-2xl font-bold text-gray-900">{project.projectInfo.progress}%</div>
                        <div className="text-xs text-gray-500">已筹集 / 目标</div>
                     </div>
                     <div className="w-full bg-gray-200 rounded-full h-2 mb-2 overflow-hidden">
                        <div className="bg-blue-600 h-full rounded-full" style={{ width: `${project.projectInfo.progress}%` }}></div>
                     </div>
                     <div className="flex justify-between text-xs text-gray-500">
                        <span>¥{project.projectInfo.currentAmount?.toLocaleString()}</span>
                        <span>¥{project.projectInfo.targetAmount?.toLocaleString()}</span>
                     </div>
                  </div>
               )}
               ...
               {/* Initiator */}
               <div className="flex items-center space-x-3 mb-6">
                  <img src={project.user.avatar} className="w-12 h-12 rounded-full" />
                  <div>
                     <div className="font-bold text-gray-900">{project.user.name}</div>
                     <div className="text-xs text-gray-500">发起人 · 信用分 780</div>
                  </div>
                  <button className="ml-auto border border-blue-600 text-blue-600 text-xs px-3 py-1.5 rounded-full font-medium">查看主页</button>
               </div>
               ...
               <h3 className="font-bold text-lg text-gray-900 mb-3">需求岗位</h3>
               <div className="space-y-2 mb-6">
                  {project.projectInfo?.roles?.map(role => (
                     <div key={role} className="flex justify-between items-center bg-gray-50 p-3 rounded-lg">
                        <span className="text-sm font-medium">{role}</span>
                        <span className="text-blue-600 text-xs">申请 &gt;</span>
                     </div>
                  ))}
               </div>
            </div>
         ...
         <div className="p-3 border-t flex space-x-3 pb-safe">
            <button className="flex-1 bg-white border border-gray-300 text-gray-900 py-2.5 rounded-full font-medium text-sm">
               收藏项目
            </button>
...
export const UserProfile = ({ params }: { params: { id: string } }) => {
  ...
  const renderTabContent = () => {
    switch (activeTab) {
       case 'works':
          return (
             <div className="p-1 grid grid-cols-2 gap-1 pb-4">
                {MOCK_COMMUNITY_FEED.map((item, idx) => (
                   <div key={idx} className="bg-gray-100 aspect-[3/4] relative rounded overflow-hidden cursor-pointer" onClick={() => pushScreen({ name: 'community_detail', params: { postId: item.id } })}>
                     {item.images && <img src={item.images[0]} className="absolute inset-0 w-full h-full object-cover" />}
                     <div className="absolute bottom-2 left-2 text-white text-xs z-10 font-bold">{item.stats.likes} 赞</div>
       ...
       case 'liked':
          return (
            <div className="text-center py-8 text-gray-400 text-sm">暂无公开的赞过内容</div>
          );
       case 'forward':
          return (
             <div className="text-center py-8 text-gray-400 text-sm">暂无转发内容</div>
          );
       case 'comments':
          return (
             <div className="text-center py-8 text-gray-400 text-sm">暂无评论记录</div>
          );
       default: return null;
    }
  };
  ...
  return (
    <div className="flex flex-col h-full bg-white relative">
      <div className="h-48 bg-gradient-to-r from-blue-300 to-purple-300 relative">
        <button onClick={popScreen} className="absolute top-4 left-4 p-2 bg-black/20 text-white rounded-full backdrop-blur-sm">
          <ChevronLeft size={20} />
        </button>
        <div className="absolute top-4 right-4 flex space-x-2">
           <button className="p-2 bg-black/20 text-white rounded-full backdrop-blur-sm"><MoreHorizontal size={20} /></button>
        </div>
      </div>

      <div className="flex-1 -mt-10 bg-white rounded-t-3xl px-6 relative overflow-y-auto">
         <div className="w-24 h-24 rounded-full border-4 border-white shadow-md bg-gray-200 -mt-12 mb-3 overflow-hidden">
           <img src={user.avatar} className="w-full h-full object-cover" alt="avatar" />
         </div>

         <div className="flex items-center space-x-2 mb-1">
           <h1 className="text-2xl font-bold text-gray-900">{user.name}</h1>
           {user.isVip && <span className="text-amber-500 font-bold italic text-lg">V</span>}
         </div>
         
         <p className="text-sm text-gray-500 mb-3">{user.bio}</p>

         <div className="flex space-x-2 mb-5">
           <span className="bg-blue-50 text-blue-500 text-xs px-2 py-1 rounded">官方合作</span>
           <span className="bg-gray-100 text-gray-500 text-xs px-2 py-1 rounded">{user.location || '未知地区'}</span>
         </div>

         <div className="flex justify-start space-x-8 mb-6 border-b pb-4">
            <div className="text-center"><div className="font-bold text-lg">{user.stats?.likes || 0}</div><div className="text-xs text-gray-400">获赞</div></div>
            <div className="text-center"><div className="font-bold text-lg">{user.stats?.followers || 0}</div><div className="text-xs text-gray-400">粉丝</div></div>
            <div className="text-center"><div className="font-bold text-lg">{user.stats?.following || 0}</div><div className="text-xs text-gray-400">关注</div></div>
         </div>

         {!isMe && (
           <div className="flex space-x-3 mb-6">
             <button className="flex-1 bg-blue-600 text-white py-2 rounded-full font-medium flex items-center justify-center">
               <PlusCircle size={18} className="mr-1" /> 关注
             </button>
             <button 
                onClick={() => pushScreen({ name: 'chat_detail', params: { title: user.name } })}
                className="flex-1 bg-gray-100 text-gray-700 py-2 rounded-full font-medium"
             >
               发消息
             </button>
           </div>
         ...
         <div className="flex justify-between border-b text-sm font-medium text-gray-500 mb-4 overflow-x-auto no-scrollbar">
            {[
              { id: 'works', label: '作品' },
              ...
              <span 
                key={tab.id} 
                onClick={() => setActiveTab(tab.id as any)}
                className={`pb-2 px-2 whitespace-nowrap cursor-pointer transition-colors ${activeTab === tab.id ? 'text-gray-900 border-b-2 border-gray-900' : ''}`}
              >
                {tab.label}
              </span>
            ))}
